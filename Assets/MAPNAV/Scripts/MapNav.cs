...
public class MapNav : MonoBehaviour 
{
	...
	public string key = "AIzaSyB_6z0XrSqzoM2npAgUJKijiYuJIf8_SyQ";  //AppKey (API key) code obtained from your maps provider (MapQuest, Google, etc.). 
	...
	void Awake(){
		...
		cam = GameObject.FindGameObjectWithTag("MapCamera").transform;
		mycam = cam.gameObject.GetComponent<Camera>();
	...
	IEnumerator MapPosition(){
	   ...
	   //------------------------------------------------------------------	//
		...
		url= "http://maps.google.com/maps/api/staticmap?center="+fixLat+","+fixLon+"&zoom="+zoom+"&scale=2&size=640x640&format=jpg&maptype=satellite&sensor=false&key="+key;
		tempLat = fixLat;
		tempLon = fixLon;
